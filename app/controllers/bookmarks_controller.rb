class BookmarksController < ApplicationController
  def index
    ...
    @bookmarks = @q.result(:distinct => true).includes(:user, :blogpost).page(params[:page]).per(10)
  ...
  def create_row_from_blogpost
    ...
    if @bookmark.valid?
      ...
      redirect_to("/blogposts/#{@bookmark.blogpost_id}", notice: "Bookmark created successfully.")
    else
      render("bookmark_templates/new_form_with_errors.html.erb")
    end
  end
  ...
  def destroy_row_from_blogpost
    ...
    redirect_to("/blogposts/#{@bookmark.blogpost_id}", notice: "Bookmark deleted successfully.")
  end
